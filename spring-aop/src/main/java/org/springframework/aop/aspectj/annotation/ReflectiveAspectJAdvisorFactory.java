...
public class ReflectiveAspectJAdvisorFactory extends AbstractAspectJAdvisorFactory implements Serializable {
	 ...
	 * <ol>
	 *     根据注解类型生成与之对应的通知对象.
	 * <li> 从目标 bean 中获取不包含 Pointcut 注解的方法列表.
	 * <li> 遍历上一步获取的方法列表, 并调用 getAdvisor 获取当前方法对应的 Advisor.
	 * <li> 创建 AspectJExpressionPointcut 对象, 并从方法中的注解中获取表达式, 最后设置到切点对象中.
	 * <li> 创建 Advisor 实现类对象 InstantiationModelAwarePointcutAdvisorImpl.
	 * <li> 调用 instantiateAdvice 方法构建通知.
	 * <li> 调用 getAdvice 方法, 并根据注解类型创建相应的通知.
	 * </ol>
	 */
	@Override
	public List<Advisor> getAdvisors(MetadataAwareAspectInstanceFactory aspectInstanceFactory) {
		...
		// 获得 切面类 的非切点注解的方法(@Before, @After...), 遍历.
	...
	private AspectJExpressionPointcut getPointcut(Method candidateAdviceMethod, Class<?> candidateAspectClass) {
		...
		// 创建一个 AspectJExpressionPointcut 对象, 用于封装 切点表达式.
	...
	public Advice getAdvice(Method candidateAdviceMethod, AspectJExpressionPointcut expressionPointcut,
		...
		switch (aspectJAnnotation.getAnnotationType()) {
			case AtPointcut: // @Pointcut 忽略, 直接返回
				if (logger.isDebugEnabled()) {
